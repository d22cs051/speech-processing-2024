...
df = pd.read_csv('../../data/VoxCeleb/list_test_hard.txt', delimiter=' ', header=None, names=['label', 'person1', 'person2'],index_col=False)
...
for model_name in model_names:
    ...
    for i in tqdm(range(0, len(df), batch_size), desc=f"Processing {model_name}"):
        ...
        for index, row in batch.iterrows():
            file_path_1 = f"/DATA1/bikash_dutta/CS/SP/A2/UniSpeech/data/VoxCeleb/dev/wav/{row['person1']}"
            file_path_2 = f"/DATA1/bikash_dutta/CS/SP/A2/UniSpeech/data/VoxCeleb/dev/wav/{row['person2']}"
            if not os.path.exists(file_path_1): # if file is not in val split
                file_path_1 = f"/DATA1/bikash_dutta/CS/SP/A2/UniSpeech/data/VoxCeleb/test/vox1_test_wav/wav/{row['person1']}"
            ...
            if not os.path.exists(file_path_2): # if file is not in val split
                file_path_2 = f"/DATA1/bikash_dutta/CS/SP/A2/UniSpeech/data/VoxCeleb/test/vox1_test_wav/wav/{row['person2']}"
...
df.to_csv(f"verification_scores_vox_batch_{batch_size}.csv",index=False)